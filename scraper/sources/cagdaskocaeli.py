"""
Çağdaş Kocaeli (cagdaskocaeli.com.tr) haber scraper modülü.
...
KAYNAK_SAYFALAR = [
    ...
    BASE_URL + "/saglik",
    BASE_URL + "/cevre",
    BASE_URL + "/kultur-sanat",
]

# Geçerli haber URL kalıpları
HABER_URL_PATTERN = re.compile(
    r"^https?://www\.cagdaskocaeli\.com\.tr/(haber|foto|video)/\d+/.+"
)
...
def _tarih_parse(tarih_str: Optional[str]) -> Optional[datetime]:
    ...
    if not tarih_str:
        return None
    try:
        # Python 3.7+ fromisoformat bazı formatları desteklemez,
        # örn. "+03:00" sorunlu olabilir → elle düzeltme
...
def scrape() -> Dict[str, int]:
    """Çağdaş Kocaeli sitesini scrape eder.

    Returns:
        { "toplam_link": int, "atlanan_mevcut": int,
          "atlanan_tarih": int, "atlanan_hata": int, "eklenen": int }
    """
    logger.info("═══ Çağdaş Kocaeli scraping başlatılıyor ═══")
    ...
    logger.info(
        "═══ Çağdaş Kocaeli scraping tamamlandı ═══\n"
        "  Toplam link  : %d\n"
        "  Eklenen      : %d\n"
        "  Mevcut (atl.): %d\n"
        "  Tarih dışı   : %d\n"
