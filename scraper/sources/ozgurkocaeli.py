"""
Özgür Kocaeli (ozgurkocaeli.com.tr) haber scraper modülü.
...
SITE_ADI = "ozgurkocaeli.com.tr"

# Haber linkleri toplanacak sayfalar
KAYNAK_SAYFALAR = [
    BASE_URL + "/",
    BASE_URL + "/kocaeli-haberleri",
...
HABER_URL_PATTERN = re.compile(
    r"^https?://www\.ozgurkocaeli\.com\.tr/(haber|foto|video)/\d+/.+"
)
...
def scrape() -> Dict[str, int]:
    """Özgür Kocaeli sitesini scrape eder.

    Returns:
        { "toplam_link": int, "atlanan_mevcut": int,
          "atlanan_tarih": int, "atlanan_hata": int, "eklenen": int }
    """
    logger.info("═══ Özgür Kocaeli scraping başlatılıyor ═══")
    ...
    logger.info(
        "═══ Özgür Kocaeli scraping tamamlandı ═══\n"
        "  Toplam link  : %d\n"
        "  Eklenen      : %d\n"
        "  Mevcut (atl.): %d\n"
        "  Tarih dışı   : %d\n"
